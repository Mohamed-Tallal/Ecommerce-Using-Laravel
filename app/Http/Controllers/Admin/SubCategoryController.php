...
class SubCategoryController extends Controller
{
    public function index(){
        $subCategories = DB::table('sub_categories')->select('id','name_'.app()->getLocale().' as name')->orderBy('id','desc')->paginate('5');
        $subCategoryCount =DB::table('sub_categories')->select('id','name_'.app()->getLocale().' as name')->get();
        return view('dashboard.subcategory.index',compact('subCategories','subCategoryCount'));
    }
